...
const cxqlSyntaxReminder = "CxQL query code is always written in C#, regardless of the language prefix in the query's name (e.g. \"javascript.\" means the query inspects JavaScript source code — it does not mean the query itself is written in JavaScript). " +
	...
	"If you know a query's short name (e.g. a helper referenced by another query's source, such as Find_HSTS_Sanitize) but not its group, use search_queries to find its full Language.Group.QueryName path — do not guess at a sequence of get_query_info calls against different group names; that wastes turns and, unlike search_queries, gives you no signal about which guess (if any) is close. " +
	"You cannot modify or override Common-language query. Fix the language-specific query that calls it instead. You can create new utility queries inside your language-specific General group, eg: JavaScript.General.New_Query - these can then be invoked by other JavaScript-langauge queries."
